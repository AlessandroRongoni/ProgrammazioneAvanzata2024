...
/**
 * Query per ottenere lo storico degli aggioramenti di un grafo
 * 
 * @param graphId - L'ID del grafo per cui ottenere lo storico degli aggiornamenti.
 * @param status - Lo stato di approvazione degli aggiornamenti da ottenere.
 * @returns Una promessa che rappresenta lo storico degli aggiornamenti del grafo specificato.
 * 
 */
export async function getFilteredUpdates(graphId:number, status:string): Promise<any>{
    // Definisci la condizione di base per 'approved'
    let approvedCondition = status === 'accepted' ? true : status === 'rejected' ? false : { [Op.ne]: null };
    return await UpdateModel.findAll({
        where: {
            graph_id: graphId,
 ...
 * 
 * @param graphId - L'ID del grafo per cui ottenere lo storico degli aggiornamenti.
 * @param dateFrom - La data di inizio del filtro. Superiore a questa data.
 * @param status - Lo stato di approvazione degli aggiornamenti da ottenere.
 * @returns Una promessa che rappresenta lo storico degli aggiornamenti del grafo specificato.
 * 
 */
export async function getFilteredUpdatesFrom(graphId:number, status:string,dateFrom:string): Promise<any>{
    // Definisci la condizione di base per 'approved'
    let approvedCondition = status === 'accepted' ? true : status === 'rejected' ? false : { [Op.ne]: null };
    return await UpdateModel.findAll({
        where: {
 ...
 * 
 * @param graphId - L'ID del grafo per cui ottenere lo storico degli aggiornamenti.
 * @param dateTo - La data di fine del filtro. Inferiore a questa data.
 * @param status - Lo stato di approvazione degli aggiornamenti da ottenere.
 * @returns Una promessa che rappresenta lo storico degli aggiornamenti del grafo specificato.
 * 
 */
export async function getFilteredUpdatesTo(graphId:number, status:string,dateTo:string): Promise<any>{
    // Definisci la condizione di base per 'approved'
    let approvedCondition = status === 'accepted' ? true : status === 'rejected' ? false : { [Op.ne]: null };
    return await UpdateModel.findAll({
        where: {
 ...
 * 
 */
export async function getFilteredUpdatesBetween(graphId:number, status:string,dateFrom:string,dateTo:string): Promise<any>{
    let approvedCondition = status === 'accepted' ? true : status === 'rejected' ? false : { [Op.ne]: null };
    return await UpdateModel.findAll({
        where: {
            graph_id: graphId,
